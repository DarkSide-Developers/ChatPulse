...
class MessageHandler {
    ...
    constructor(client) {
        ...
        this.rateLimiter = new RateLimiter({
            limits: {
                message: { requests: 20, window: 60000 },
                media: { requests: 10, window: 60000 }
            }
        });
    }
    ...
    async sendMessage(chatId, message, options = {}) {
        ...
        // Check rate limits
        const rateLimitResult = await this.rateLimiter.checkLimit('message', chatId);
    ...
    async _sendAdvancedMessage(chatId, content, type = 'text', options = {}) {
        try {
            ...
            await this.rateLimiter.consume(type === 'text' ? 'message' : 'media', chatId);
